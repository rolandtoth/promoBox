 ...
 *
 */
if(!Array.prototype.indexOf)Array.prototype.indexOf=function(c,d){var a=d||0,b=this.length;for(a;a<b;a=a+1)if(this[a]===
c)return a;return-1};var PBlib=(function(){"use strict";return{cookie:{createCookie:function(d,e,b){var c="",a;if(b&&b
...
(Math.random()*(1/a));return b===0}}})(),promoBox=function(k){"use strict";var j,r,t,s=document.getElementsByTagName(
"head")[0],q=document.body,o=window.location.href;j={addStyles:function(){if(k.disableStyles)return false;var a=[
...
"body { overflow: hidden; }";if(window.innerHeight<q.scrollHeight)a+="body { width: "+q.offsetWidth+"px; }"}j.styles=
PBlib.makeElement("style",{id:"promoStyle",type:"text/css"});if(j.styles.styleSheet)j.styles.styleSheet.cssText=a;else j
.styles.appendChild(document.createTextNode(a));if(s.firstChild)s.insertBefore(j.styles,s.firstChild);else s.appendChild
...
disableCloseButton||null;k.disableOverlay=k.disableOverlay||null;k.disableOverlayClose=k.disableOverlayClose||null;k.
disableStyles=k.disableStyles||null;k.endDate=k.endDate||null;k.fadeInDuration=k.fadeInDuration||0;k.fadeOutDuration=k.
fadeOutDuration||0;k.forceOnUrl=k.forceOnUrl||null;k.hideOnUrl=k.hideOnUrl||null;k.imagePath=k.imagePath||null;k.
interstitialDuration=k.interstitialDuration||null;k.interstitialSkipText=k.interstitialSkipText||"Skip this ad";k.
interstitialText=k.interstitialText||"or wait %s secs";k.link=k.link||null;k.loadDelay=k.loadDelay||null;k.
randomFrequency=k.randomFrequency||1;k.showOnUrl=k.showOnUrl||null;k.showScrollbar=k.showScrollbar||false;k.startDate=k.
startDate||null;k.target=k.target||"_self";f=k.randomFrequency&&!PBlib.checkFrequency(k.randomFrequency);d=!!k.
forceOnUrl&&PBlib.findInArray(k.forceOnUrl,o);b=!!k.deleteCookieOnUrl&&PBlib.findInArray(k.deleteCookieOnUrl,o);g=(
function(a){if(!a||PBlib.isArray(a)&&!a.length)return false;if(!!a&&!PBlib.findInArray(a,o))return true})(k.showOnUrl);e
=!!k.hideOnUrl&&PBlib.findInArray(k.hideOnUrl,o);if(!k.imagePath)return false;if(!d){if(f)return false;if(k.startDate){k
...
cookieLifetime){if(PBlib.cookie.readCookie("promoBox"))return false;if(!b)PBlib.cookie.createCookie("promoBox","1",k.
cookieLifetime)}if(g)return false;if(e)return false}if(b)PBlib.cookie.eraseCookie("promoBox");if(k.interstitialDuration)
{k.disableOverlay=false;k.disableOverlayClose=true;k.disableCloseButton=true;k.closeButtonText=null;k.autoCloseSeconds=k
...
promo.container,k.className);this.promo.container.appendChild(this.promo.content);q.appendChild(this.promo.container)},
...
keyClose);if(k.interstitialDuration){PBlib.addEvent(this.promo.interstitialSkipText,"click",this.events.close);t=
...
container)q.removeChild(j.promo.container);if(j.styles)j.styles.parentNode.removeChild(j.styles);j=null},close:function(
...
fadeOutDuration*1000)}else j.events.destroyPromo();if(r)clearTimeout(r);if(t)clearInterval(t)},autoClose:function(){r=
setTimeout(function(){j.events.close()},k.autoCloseSeconds*1000)},keyClose:function(a){if(j&&a.keyCode===27)j.events.
close()},openLink:function(a,b,c){if(a)PBlib.preventDefault(a);PBlib.callCallBack("promoBoxClick");if(b.indexOf(
"#promoClose")===-1)window.open(b,c);j.events.close()}},startPromo:function(){PBlib.callCallBack("promoBoxStart");j.
addStyles();j.buildPromo();j.addListeners()}};if(j.init())PBlib.loadSprite(k.imagePath,function(){window.
promoBoxInstanceID=clearTimeout(window.promoBoxInstanceID)||setTimeout(function(){j.startPromo()},k.loadDelay*1000)})}