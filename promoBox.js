var promoBox = function (o) {
    ...
    var helpers = {
        ...
        callCallBack: function(fx) {
	        if (typeof window[fx] === 'function') { window[fx](); }
        }
    };
    ...
    var PB = {
        ...
        init: function () {
            ...
            if (o.daysToSeeAgain) {
                if (helpers.cookie.readCookie('promoBox')) { return false; }
        ...
        buildPromo: function () {
            ...
            if (!o.disableOverlay) { this.promo.container.appendChild(this.promo.overlay); }
            if (!o.disableCloseButton) { this.promo.content.appendChild(this.promo.close); }
            this.promo.container.appendChild(this.promo.content);
            document.body.appendChild(this.promo.container);
        },
        addListeners: function () {
            if (!o.disableCloseButton) { helpers.addEvent(this.promo.close, 'click', this.events.close); }
            if (!o.disableOverlayClose) { helpers.addEvent(this.promo.overlay, 'click', this.events.close); }
            if (o.link) { helpers.addEvent(this.promo.image, 'click', this.events.addLink); this.promo.image.style.cursor = 'pointer'; }
            if (!o.disableKeyClose) { helpers.addEvent(document, 'keydown', this.events.keyClose); }
        ...
        events: {
            close: function () {
                ...
                helpers.callCallBack(o.closeCallback);
                if (PB.events.autoCloseID) { clearTimeout(PB.events.autoCloseID); }
                PB = null;
            },
            autoClose: function () {
            ...
            addLink: function () {
                helpers.callCallBack(o.clickCallback);
                if (o.target === '') {
    ...
    if (PB.init()) {
        helpers.callCallBack(o.startCallback);
        PB.buildPromo();
        PB.addListeners();
        if (!o.disableStyles) { PB.addStyles(); }
    }
};
